...
urlpatterns = [
    ...
    url(
        r"^baskets/(?P<basket_id>[0-9]+)/add/(?P<variable_id>[0-9]+)/$",
        add_variable,
        name="add_variable",
    ),
    url(
        r"^baskets/(?P<basket_id>[0-9]+)/remove/(?P<variable_id>[0-9]+)/$",
        remove_variable,
        name="remove_variable",
    ),
    url(
        r"^baskets/(?P<basket_id>[0-9]+)/add_concept/(?P<concept_id>[0-9]+)/$",
        add_concept,
        name="add_concept",
    ),
    url(
        r"^baskets/(?P<basket_id>[0-9]+)/remove_concept/(?P<concept_id>[0-9]+)/$",
        remove_concept,
        name="remove_concept",
    ),
    url(r"^baskets/(?P<basket_id>[0-9]+)$", basket_detail, name="basket"),
    url(
        r"^baskets/(?P<basket_id>[0-9]+)/scripts/new/(?P<generator_name>[0-9a-z_-]+)$",
        script_new_lang,
    ),
    url(r"^baskets/(?P<basket_id>[0-9]+)/scripts/new$", script_new),
    url(
        r"^baskets/(?P<basket_id>[0-9]+)/scripts/(?P<script_id>[0-9]+)/delete$",
        script_delete,
    ),
    url(
        r"^baskets/(?P<basket_id>[0-9]+)/scripts/(?P<script_id>[0-9]+)$",
        script_detail,
        name="script_detail",
    ),
    url(r"^baskets/(?P<basket_id>[0-9]+)/scripts/(?P<script_id>[0-9]+)/raw$", script_raw),
]
