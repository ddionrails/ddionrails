...
class SoepMixin:
    ...
    def _generate_script_dict(self):
        ...
        for variable in self.basket.variables.all():
            ...
            if not dataset_name in script_dict.keys():
                script_dict[dataset_name] = self._create_dataset_dict(dataset_name, variable)
    ...
    def _get_selected_years(self, script_dict):
        return set([
            d["prefix"] for d in script_dict.values()
        ])
...
class SoepStata(ScriptConfig, SoepMixin):
    ...
    def __init__(self, script, basket):
        ...
        self.fields = [
            ...
            dict(name="analysis_unit", label="Analysis Unit",
                scale="select", options=dict(p="Individual", h="Household"),),
            dict(name="private", label="Private households",
                scale="select", options=dict(
                    t="Private households only",
                    f="All households"),
                ),
            dict(name="balanced", label="Sample composition",
                scale="select", options=dict(t="balanced", f="unbalanced"),),
            dict(name="age_group", label="Age group",
                scale="select", options=dict(
        ...
        self.script_dict = { x:y for x, y in self.script_dict_raw.items() if x in valid_datasets }
    ...
    def _render_disclaimer(self):
        script  = "\n%s --------------------------------------------------------------------." % self.COMMENT
        script += "\n%s This command file was generated by paneldata.org                    ." % self.COMMENT
        script += "\n%s --------------------------------------------------------------------." % self.COMMENT
        script += "\n%s !!! I M P O R T A N T - W A R N I N G !!!                           ." % self.COMMENT
        script += "\n%s You alone are responsible for contents and appropriate.             ." % self.COMMENT
        script += "\n%s usage by accepting the usage agreement.                             ." % self.COMMENT
        script += "\n%s --------------------------------------------------------------------." % self.COMMENT
        script += "\n%s Please report any errors of the code generated here                 ." % self.COMMENT
        script += "\n%s to soepmail@diw.de                                                  ." % self.COMMENT
        script += "\n%s --------------------------------------------------------------------." % self.COMMENT
        script += "\n"
        return script
...
class SoepR(SoepStata):
    ...
    def _render_read_data(self):
        ...
        for dataset in self.script_dict.values():
            ...
            script += "\ntmp_dataset <- read.dta(file.path(path_in, \"%s.dta\"), convert.factors=F)" % dataset["name"]
