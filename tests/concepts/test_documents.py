...
class TestDocuments(TransactionTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.variable_with_concept = VariableFactory(
            concept=self.concept, dataset__study=self.study
        )
    ...
    def test_search_concept_by_label_de(self):
        result = (
            ConceptDocument.search()
            .query("match", label_de=self.concept.label_de)
            .count()
        )
        expected = 1
        assert expected == result
