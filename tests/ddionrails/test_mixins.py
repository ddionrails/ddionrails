...
class TestModelMixin:
    ...
    def test_get_or_create_method(self, mocker):
        with mocker.patch('tests.ddionrails.test_mixins.MixinChild.objects.get_or_create', return_value=(1, 1)):
    ...
    def test_get_method_success(self, mocker):
        with mocker.patch('tests.ddionrails.test_mixins.MixinChild.objects.get', return_value=True):
            dictionary = dict(name='some-name')
