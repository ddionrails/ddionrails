...
""" ModelForms for ddionrails.instruments app """
...
class InstrumentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "study" not in self.data.keys():
            self.data["study"] = Study.objects.get(name=self.data.get("study_name")).pk
...
class QuestionForm(forms.ModelForm):
    ...
    def _set_instrument(self):
        if "instrument" in self.data.keys():
            ...
            study = Study.objects.get(name=self.data.get("study_name"))
            self.data["instrument_object"] = Instrument.objects.get(
                study=study, name=self.data.get("instrument_name")
            )
